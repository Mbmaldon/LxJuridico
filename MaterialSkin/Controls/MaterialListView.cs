...
namespace MaterialSkin.Controls
{
	public class MaterialListView : ListView, IMaterialControl
	{
		...
		{
            int wImage = SmallImageList != null ? SmallImageList.ImageSize.Width : 0;
			...
			foreach (ListViewItem.ListViewSubItem subItem in e.Item.SubItems)
			{
                ...
                //				 new Rectangle(subItem.Bounds.X + ITEM_PADDING + LargeImageList.Images[e.Item.ImageIndex].Width, ITEM_PADDING, subItem.Bounds.Width - 2 * ITEM_PADDING, subItem.Bounds.Height - 2 * ITEM_PADDING),
                //				 getStringFormat());
                g.DrawString(subItem.Text, SkinManager.ROBOTO_MEDIUM_10, SkinManager.GetPrimaryTextBrush(),
                                 new Rectangle(subItem.Bounds.X + ITEM_PADDING + wImage, ITEM_PADDING, subItem.Bounds.Width - 2 * ITEM_PADDING, subItem.Bounds.Height - 2 * ITEM_PADDING),
                                 getStringFormat());
            }

            //b = (Bitmap)LargeImageList.Images[e.Item.ImageIndex] + b;


            //e.Graphics.DrawImage((Image)b.Clone(), new Point(0, e.Item.Bounds.Location.Y));
            e.Graphics.DrawImage((Image)b.Clone(), new Point(0, e.Item.Bounds.Location.Y));
            if (SmallImageList != null)
                e.Graphics.DrawImage((Image)SmallImageList.Images[e.Item.ImageIndex].Clone(), new Point(0, e.Item.Bounds.Location.Y + 5));

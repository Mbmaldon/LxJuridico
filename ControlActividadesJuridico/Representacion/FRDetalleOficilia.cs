...
{
    public partial class FRDetalleOficilia : Form
    {
        int     iIdSol;
        int     iContador;
        ...
        public FRDetalleOficilia(int iIdSolicitud)
        {
            InitializeComponent();
            iIdSol  = iIdSolicitud;
            detalle = null;
        ...
        /*Evento CellContentClick para lanzar modal segun el click en la columna Editar del datagrid*/
        private void dgvOficialias_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (this.dgvOficialias.Columns[e.ColumnIndex].Name.Equals("Editar"))
                {
                    FRRRecepcionOficialia aFormulario = new FRRRecepcionOficialia(detalle.sNumCliente,detalle.sIdCaso, int.Parse(dgvOficialias.SelectedCells[0].FormattedValue.ToString()));
        ...
        /*Evento CellPainting de datagrid para colocar imagen en la columna Editar*/
        private void dgvOficialias_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.ColumnIndex >= 0 && this.dgvOficialias.Columns[e.ColumnIndex].Name == "Editar" && e.RowIndex >= 0)
            {
                e.Paint(e.CellBounds, DataGridViewPaintParts.All);

                DataGridViewButtonCell celBoton = this.dgvOficialias.Rows[e.RowIndex].Cells["Editar"] as DataGridViewButtonCell;
        ...
        /*Evento CellMouseMove para cambiar el cursor cuando pase por la columna Editar del data grid*/
        private void dgvOficialias_CellMouseMove(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (dgvOficialias.Columns[e.ColumnIndex].Name.ToString() == "Editar")
            {

...
{
	public partial class frmObligacionesPFisica : Form
	{
        ...
        private void btnContinuar_Click(Object sender, EventArgs e)
		{
            ...
            foreach (Control ctrl in panel4.Controls)
            {
                ...
                {
                    LollipopRadioButton chkb = ctrl as LollipopRadioButton;
                    if (chkb.Checked)
                    {
                        i++;
                        switch (chkb.Name)
            ...
            if (i > 0)
			{
				...
				List<DetalleObligacion> lista = new DetalleObligacion().GetListObligaciones(bNueva ? 10 : 0, // 16
																							bExistente ? 11 : 0, // 17
																							0,
																							0,
																							0,
                ...
                List<DetalleObligacion> listaDictamenes = new DetalleObligacion().GetListDictamenes(14); //bNueva ? 14 : 15
                cargarDictamenes(pnlDictamenes, listaDictamenes, 20);

                List<DetalleObligacion> listaISN = new DetalleObligacion().GetListISN(10); //bNueva ? 10 : 11
                cargarISN(pnlISN, listaISN, 20);

                btnGuardar.Visible			= true;

				int iUbicacion = 571;//480;//476;//580;//512

				if (bNueva)
				{
					btnVHonorarios.Visible	= true;
					btnVHonorarios.Location	= new Point(iUbicacion, 101); // 434, 67 //4
				...
				if (bExistente)
				{
					...
					btnVArrendamiento.Location	= new Point(iUbicacion, 101);
					iUbicacion					= iUbicacion + 50;
				}
			}
			else
        ...
        private void btnModificarRegimen_Click(Object sender, EventArgs e)
		{
			...
			lblTitulo.Location	= new Point(259, 253);//208, 82
        ...
        /// <param name="iTipo">Tipo de declaración, 1 para obligaciones fiscales, 2 para dictamenes y 3 para ISN</param>
        /// <returns></returns>
		public bool GuardarObligaciones(Panel pnlListaObligaciones, int iTipo)
		{
			...
			try
			{
				...
				{
					// RECORRE LA LISTA DE OBLIGACIONES Y GUARDA LAS QUE SE ENCUENTREN MARCADAS EN LA BD
					foreach (Control control in pnlListaObligaciones.Controls)
					{
						if (control is LollipopCheckBox)
						{
        ...
        public void cargarDictamenes(Panel panel, List<DetalleObligacion> dictamenes, int iSeparacion)
        {
            ...
            for (int i = 0; i < dictamenes.Count; i++)
            {
                ...
                if (dictamenes[i].iR01 != null)
                {
                    ...
                    chkbR1.Size         = new Size(20, 20);
                    chkbR1.Cursor       = Cursors.Hand;
                    chkbR1.CheckColor   = bNueva ? "#5d4037" : "#558b2f";
